    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && ((compiler.version).ge "8.4" && (compiler.version).lt "8.9")) (hsPkgs."temporary" or (buildDepError "temporary"))) ++ (pkgs.lib).optional (!system.isWindows) (hsPkgs."unix" or (buildDepError "unix"));
        buildable = true;
        };
      tests = {
        "unit-tests" = {
