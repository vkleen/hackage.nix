...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, config, ... }:
  {
    ...
    package = {
      specVersion = "2.0";
      identifier = { name = "ghc"; version = "8.4.1"; };
    ...
    components = {
      "library" = {
        depends = (([
          ...
          ] ++ (pkgs.lib).optional (!(system.isLinux && system.isX86_64 && (compiler.isGhc && (compiler.version).eq "8.4.1"))) (hsPkgs."base" or (buildDepError "base"))) ++ (pkgs.lib).optional (!flags.buildable) (hsPkgs."base" or (buildDepError "base"))) ++ (if system.isWindows
