...
in { system, compiler, flags, pkgs, hsPkgs, pkgconfPkgs, config, ... }:
  {
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "jsaddle"; version = "0.1.1.4"; };
      license = "MIT";
      copyright = "";
    ...
    components = {
      "library" = {
        depends = (([
          ...
          ] ++ (pkgs.lib).optional (flags.ghcjs && flags.jsffi) (hsPkgs."ghcjs-base" or (buildDepError "ghcjs-base"))) ++ (pkgs.lib).optionals (!flags.ghcjs || flags.webkit) (if flags.gtk3
          then [
